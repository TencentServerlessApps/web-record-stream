...
async function start(data, taskID, appId) {
  ...
  if (!data['Mode']) {
    data['Mode'] = 1;
    logger.log('mode no set, set default mode 1');
  } else {
    if (data['Mode'] < 1 || data['Mode'] > 3) {
      logger.log('[Error]invalid Mode');
      return {
        ErrorCode: 'InvalidParam',
        ErrorMessage: `Mode must in range [1, 3]`,
      };
    }
  }

  const enableLivePush = ((data['Mode'] & 0x02) === 0 ? 0 : 1);
  const enableRecord = ((data['Mode'] & 0x01) === 0 ? 0 : 1);
  ...
  if (
    ...
    // check live push param if enable
  if (enableLivePush) {
    if (data['Output'] && data['Output']['ServiceParam']) {
      const serviceParam = data['Output']['ServiceParam'];
      if (!serviceParam['RtmpUrl']) {
        return {
          ErrorCode: 'InvalidParam',
          ErrorMessage: `RtmpUrl must set in Output.ServiceParam`,
        }; 
      }
    } else {
      return {
        ErrorCode: 'InvalidParam',
        ErrorMessage: `ServiceParam must set in Output`,
      };
    }
  }
  ...
  if (enableRecord) {
    if (data['Output'] && data['Output']['Video']) {
      ...
      if (!outputVideoInfo['Muxer']) {
        return {
          ErrorCode: 'InvalidParam',
          ErrorMessage: `Muxer must set in Output.Video`,
        };
      }
    ...
    if (process.env.DOUBLE_RECORD === 'OPEN') {
      ...
      try {
        ...
        const d = {...data, Mode:1};
        // double task only support record
        const res = await scf.web_record(d, doubleTaskID, 'Start');
  ...
  try {
    await redisHelper.setString(taskInfoKey, JSON.stringify(taskInfo), 0);
