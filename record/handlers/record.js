...
recorder.record = async (taskInfo, requestID) => {
  ...
  let mode = taskInfo.Param.Mode || 1;
  const enableRecord = (mode & 0x01) === 0 ? false : true;
  const enableLive = (mode & 0x02) === 0 ? false: true;
  const opt = {enableRecord, enableLive};
  
  // 1. update taskinfo
  try {
    // 只有状态为normal的时候才需要将状态更新为recording
    if (taskInfo.Status == 'normal') {
      logger.log('[RECORD] update task status from normal to recording');
  ...
  let videoDir = null;
  if (enableRecord) {
    opt.recordOption = {enableRecord: enableRecord};
    ...
    videoDir = path.join(config.videoRootDir, taskID, 'raw');
    if (storageType == 'cfs'){
  ...
  if (enableLive) {
    opt.liveOption = {enableLive: enableLive}
    opt.liveOption.livePushUrl = taskInfo.Param.Output.ServiceParam.RtmpUrl;
    opt.liveOption.frameRate = taskInfo.Param.Output.ServiceParam.FrameRate;
  }

  logger.log(`start record mode=${mode}, opt=${JSON.stringify(opt)}`);
        ...
        try {
          await backgroundPage.evaluate(
            (tID, recUrl, option) => {
              ...
              startRecording(tID, recUrl, option);
      ...
      if (backgroundPage) {
        try {
          await backgroundPage.evaluate(
            (tid, recURL, option) => {
              // eslint-disable-next-line no-undef
              resumeRecording(tid, recURL, option);
              return Promise.resolve(0);
            },
            taskID,
            url,
  ...
  let loadTimeout = process.env.PAGE_LOAD_TIMEOUT === 'undefined' ? 60000 : process.env.PAGE_LOAD_TIMEOUT;
  await page.goto(url, {
    waitUntil: 'load',
    timeout: loadTimeout,
  });
  ...
  async function uploadSlice(key, buffer) {
    ...
    await backgroundPage.exposeFunction('sendData', (data) => {
      ...
      if (storageType == 'cfs') {
        ...
        if (
          ...
          const p = uploadSlice(key, buffer);
          promises.push(p);
        }
      }
    });
  ...
  try {
    await backgroundPage.exposeFunction('onLiveEvent', (event, data) => {
      if (!enableLive) {
        return;
      }
      logger.log(`[LIVE] on live event update, PublishStatus=${event}, PublishMessage=${JSON.stringify(data)}`);
      // PublishStatus
      redisHelper.updateTaskInfo(redisHelper.getTaskInfoKey(taskID), {
        PublishStatus: event,
        PublishMessage: data
      }).then(()=>{
        logger.log(`[LIVE] on live event update success, PublishStatus=${event}, PublishMessage=${JSON.stringify(data)}`);
      });
   
    });
  } catch (err) {
    if (!(err.message && err.message.includes('already exists'))) {
      logger.log('expose live event function to backgroup page failed, err ', err);
    }
  }
  ...
  logger.log('waiting for flushing av data to cfs...');
  ...
  if (bufferSize > 0) {
    const key = getSliceKey(taskID, requestStartTs, sliceStartTs);
    await uploadSlice(key, Buffer.concat(bufferList));
  }

  logger.log('av data flush to cfs finished');

  if (enableRecord) {
    logger.log(`enableRecord=${enableRecord}, invokeTranscode`);
    // 6. invoke scf:transcode & update taskInfo & add heartbeat ts
    await recorder.invokeTranscode(taskInfo, requestID);
  } else {
    logger.log(`enableRecord=${enableRecord}, skip invokeTranscode`);
    // 4. remove record heartbeat
    await removeHeartbeat('record', taskID, requestID, heartbeatWorker, null, 'status-change');
  }
};
